...
// Vite doesn't support webworker-loader like webpack
...
class WebWork {
    ...
    start() {
        ...
        // Vite compatibility: mock worker for development
        this.worker = {
            postMessage: (data) => {
                console.log("Worker would receive:", data)
                // Mock response after timeout to simulate worker
                setTimeout(() => {
                    if (data.id) {
                        this.onmessage({
